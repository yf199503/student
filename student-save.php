...
if( $action == "add"){
	...
	$sql = "insert into student(学号,班号,照片,姓名,性别,出生日期,电话) value('{$sID}','{$bjNumber}','{$filename}','{$sName}',{$sSex},'{$sBrithday}','{$sPhone}')";
}else if($action == "update"){
	//修改记录
	$str = "数据修改成功！";
	$str2 = "数据修改失败！";
	$url = "student-list.php";
	$sql = "update student set 学号='{$sID}',班号='{$bjNumber}',照片='{$filename}',姓名='{$sName}',性别={$sSex},出生日期='{$sBrithday}',电话='{$sPhone}' where id={$id}";
}else{
